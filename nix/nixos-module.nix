  ...
  config =
    ...
    lib.mkMerge [
      ...
      (lib.mkIf clientCfg.enable {
        environment.systemPackages = [ clientCfg.package ];
        environment.variables.PASTE_ENDPOINT = clientCfg.endpoint;
      })

      (lib.mkIf (clientCfg.enable && clientCfg.private.user != "") {
        environment.variables.PASTE_USER = clientCfg.private.user;
      })

      (lib.mkIf (clientCfg.enable && clientCfg.private.userFile != null) {
        environment.variables.PASTE_USER_FILE = clientCfg.private.userFile;
      })

      (lib.mkIf (clientCfg.enable && clientCfg.private.pass != "") {
        environment.variables.PASTE_PASS = clientCfg.private.pass;
      })

      (lib.mkIf (clientCfg.enable && clientCfg.private.passFile != null) {
        environment.variables.PASTE_PASS_FILE = clientCfg.private.passFile;
      })
